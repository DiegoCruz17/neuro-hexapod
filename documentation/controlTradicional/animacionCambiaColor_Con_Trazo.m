function hexapod_radial_sim_con_sliders_con_rastro()
    ...
    %% Animación
    k_total = 0:0.05:30*pi;
    
    % Inicializar estructuras para rastros
    rastros = cell(6,1);
    for i = 1:6
        rastros{i} = []; % cada rastro es una matriz 3xN
    end

v = VideoWriter('hexapod_sim.mp4', 'MPEG-4');
v.FrameRate = 20;
open(v);
    ...
    while ishandle(f)
        ...
        for k = k_total
            ...
            for i = 1:6
                ...
                % Almacenar posición final del efector para rastro
                rastros{i} = [rastros{i}, P3];

                % Determinar color según la fase de marcha
                fase = mod(k+pi/2, 2*pi);
                if mod(i,2) == 1  % pata impar
                    if fase > pi
                        color_segmento = 'b';
                    else
                        color_segmento = 'r';
                ...
                plot3(P3(1), P3(2), P3(3), 'o', 'MarkerFaceColor', color_segmento, 'MarkerEdgeColor', 'k', 'MarkerSize', 6);

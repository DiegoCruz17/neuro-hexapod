    ...
    while ishandle(f)
        for k = k_total
            ...
            for i = 1:6
                ...
                target = patas{i}';
                fprintf('Pata %d - Target: [%.2f, %.2f, %.2f]\n', i, target(1), target(2), target(3));
                % Cinemática inversa
...
function [theta1, theta2, theta3] = inverse_kinematics_hexapod(base, target, L0, L1, L2)
    ...
    % Ángulo base (hombro horizontal)
    theta1 = atan2d(dy, dx);
    Rz = rotz(theta1)'
    
    % Transformar al sistema local de la pierna
    local = Rz * [dx; dy; dz];
    x_local = local(1) - L0  % distancia desde el hombro
    z_local = local(3) ;      % altura respecto a base
    ...
    % Siempre usar codo arriba: θ3 negativo
    theta3 = acosd(D);
    
    % Calcular θ2 con ley de cosenos y ley de senos
    alpha = atan2d(z_local, x_local);
    beta = atan2d(L2 * sind(-theta3), L1 + L2 * cosd(-theta3));
    theta2 = -(alpha - beta);  % signo menos para levantar hacia arriba
end


function pos = forward_kinematics(base, t1, t2, t3, L0, L1, L2)
    % Cinemática directa para obtener la posición final del efector
    P0 = base';
    Rz = rotz(t1);
    P1 = P0 + Rz * [L0; 0; 0];
